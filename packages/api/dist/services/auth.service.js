...
async function login(data) {
    ...
    // 3. 비밀번호가 일치하면, JWT 생성
    const token = jwt.sign({ userId: user.id, role: user.role }, // 토큰에 담을 정보
...
async function handleKakaoLogin(code) {
    ...
    const ServiceToken = jwt.sign({ userId: user.id, role: user.role }, process.env.JWT_SECRET, { expiresIn: '1d' });
